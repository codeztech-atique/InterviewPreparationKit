...
const defineParking = new Parking_Lot(2, 0);


const allocateParkingForCar = defineParking.allocatedParking(VehicalType.CAR);

const allocateParkingForBike = defineParking.allocatedParking(VehicalType.BIKE);
...
if(allocateParkingForCar) {
  ...
  console.log("Car:",defineParking);

} else if(allocateParkingForBike) {
  const generateParkingTicket2 = defineParking.generateParkingTicket(VehicalType.BIKE);
  const parking2 = new Parking_Ticket(new Date(), generateParkingTicket2, VehicalType.BIKE);
  console.log(VehicalType.BIKE, " is parked in the parking.");
  console.log("Parking time:", parking2.getEntryTime());
  console.log("Parking ticket:", parking2.getTicketNumber());
  console.log("Vehical Type:", parking2.getVehicalType())
  console.log("Bike:",defineParking);
} else {
